...
export async function ExchangeMPoint( _id : string,
                                      _nowTime : string,
                                      _amount : number,
                                      _phoneNumber : string,
                                      _mpoint_domain : string,
                                      _mpoint_AppID : string,
                                      _mpoint_AppSecret : string ) : Promise<MPointResponse> {

    var response : MPointResponse  = new MPointResponse();

    const transactionId = `CE_${_id.toString().padStart(8, '0')}_${_amount}_${_nowTime.replace(/[^0-9]/g, "")}`;
    ...
    try {
        ...
        if (fetchResponse.ok) {
            ...
            if ( response.code == 0 ) {
                ...
                console.log("MPoint exchange successful");
